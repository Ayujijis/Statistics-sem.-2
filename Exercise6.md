...
df_gapminder = pd.read_csv("https://raw.githubusercontent.com/kirenz/datasets/refs/heads/master/gapminder.csv")
df_gapminder.head(2)
```
...
df_gapminder.rename(columns={'gdpPercap': 'gdp_cap'}, inplace=True)
...
plt.hist(df_2007['pop'])
plt.xlabel('Population')
plt.ylabel('Count')
plt.title('Distribution of Population in 2007')
plt.show()

plt.hist(df_2007['pop'], density=True)
plt.xlabel('Population')
plt.ylabel('Probability')
plt.title('Probability Distribution of Population in 2007')
plt.show()

plt.hist(np.log10(df_2007['pop']), density=True)
plt.xlabel('log10(Population)')
plt.ylabel('Probability')
plt.title('Log-scaled Probability Distribution of Population in 2007')
plt.show()
...
sns.scatterplot(data = df_gapminder[df_gapminder['year'] == 2007],
               x = 'gdp_cap_log', y = 'life_exp',
                hue = "continent", size = 'pop', alpha = .7);
```
...
![png](Exercise6_files/Exercise6_68_1.png)
